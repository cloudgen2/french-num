...
pub fn print_type_exit_to_exit_sub() {
    println!("Tapez '{}' pour terminer le programme!", "exit".red());
    println!("");
}
...
pub fn print_what_num(question: u32, quantity: u32, highlight: &str) {
    print!("{}{} {}{}{} ",
        highlight.red(), 
        format!("{})",question).blue().bold(), 
        format!("Quel est le numéro ").yellow(), 
        quantity, 
        format!(" en français?").yellow());
}

pub fn print_correct() {
    println!("» {}","C'est correct!".dimmed());
}

pub fn print_correct_ans(ans: &String, receive_ans: &String) {
    println!("» {}{}{}","'".dimmed(), receive_ans.italic(),"'.".dimmed());
    println!("» {}{}{}","La bonne réponse est '".dimmed(), ans.italic(),"'.".dimmed());
}

pub fn process_ans(input:&str) -> String{
    let re = Regex::new(r"\s*[\.,/!:;\s\?]+\s*").unwrap();
    let ans01=re.replace_all(input.trim()," ");
    let re2 = Regex::new(r"\bd'").unwrap();
    let ans02=re2.replace_all(ans01.trim(),"de ");
    let re3 = Regex::new(r"[-]+").unwrap();
    let ans03=re3.replace_all(ans02.trim()," ");
    ans03.trim().to_lowercase().to_string()
}